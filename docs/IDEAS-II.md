...
| 4 | Live permission inspector | On the market page, read live off the rollup and re-read on the permission's own subscription. Shows the market **public** and the confession **private**, each member's flags decoded from the raw byte (`0x0f` = authority, logs, balances, message) |

| 5 | VRF grace countdown | Verified against a genuinely stalled oracle: countdown live at 00:16, flip to "grace elapsed", retry button appears, clicking it moved the market from `vrfPending` to `expired` **on chain** |
